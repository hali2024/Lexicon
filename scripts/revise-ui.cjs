...
s=s.slice(0,speechStart)+`function speak(text,accent='US'){
  ...
  const playRemoteAudio=()=>{
    ...
    try{
      ...
      speechAudio=audio;audio.onerror=()=>showToast(LANG==='zh'?'发音暂时不可用，请稍后重试。':'Pronunciation unavailable. Please try again.');
      audio.play().catch(()=>showToast(LANG==='zh'?'发音暂时不可用，请稍后重试。':'Pronunciation unavailable. Please try again.'));
  ...
  if(window.speechSynthesis&&window.SpeechSynthesisUtterance){
    ...
    utterance.onerror=()=>{if(requestId===speechRequestId)playRemoteAudio();};
