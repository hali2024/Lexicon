...
s=s.slice(0,speechStart)+`function speak(text,accent='US'){
  ...
  const playRemoteAudio=()=>{
    ...
    try{
      const audio=new Audio('/api/pronunciation/tts?word='+encodeURIComponent(value)+'&accent='+accent);
      speechAudio=audio;audio.onerror=()=>playBrowserSpeech();
      audio.play().catch(()=>{
        if(requestId===speechRequestId&&window.speechSynthesis)playBrowserSpeech();
      });
    }catch{showToast(LANG==='zh'?'发音暂时不可用，请稍后重试。':'Pronunciation unavailable. Please try again.');}
  };
  const playBrowserSpeech=()=>{
    if(requestId!==speechRequestId||!window.speechSynthesis||!window.SpeechSynthesisUtterance){showToast(LANG==='zh'?'发音暂时不可用，请稍后重试。':'Pronunciation unavailable. Please try again.');return;}
    speechSynthesis.cancel();
    const utterance=new SpeechSynthesisUtterance(value);
    utterance.lang=accent==='UK'?'en-GB':'en-US';
    const voice=speechSynthesis.getVoices().find(v=>v.lang.replace('_','-').toLowerCase()===utterance.lang.toLowerCase());
